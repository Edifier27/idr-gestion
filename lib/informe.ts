...
export async function generarInforme(s: SiniestroRow, notas: string[]): Promise<string> {
  ...
  const msg = await client.messages.create({
    ...
    max_tokens: 2000,
    system:
      "Sos un perito liquidador de siniestros en Argentina. Redactás informes técnico-legales " +
      "claros, formales y objetivos. Estructura: 1) Datos del siniestro, 2) Antecedentes, " +
      "3) Verificaciones realizadas, 4) Análisis, 5) Conclusión/dictamen. Escribí en español " +
      "rioplatense formal. Es un BORRADOR para que el perito revise y ajuste; no inventes hechos " +
      "que no estén en los datos: si falta información, indicá 'a completar'.",
    messages: [
